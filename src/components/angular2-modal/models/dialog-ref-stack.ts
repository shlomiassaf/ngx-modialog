...
export class DialogRefStack {
    private _stack: DialogRef[] = [];


    push(dialogRef: DialogRef): void {
     ...
     * @param dialogRef
     */
    pushManaged(dialogRef: DialogRef): void {
        this.push(dialogRef);
        dialogRef.result
     ...
     * @param dialogRef
     */
    remove(dialogRef: DialogRef): void {
        let idx = this._stack.indexOf(dialogRef);
        if (idx > -1) this._stack.splice(idx, 1);
    }


    index(index: number): DialogRef {
        return this._stack[index];
    }

    indexOf(dialogRef: DialogRef): number {
